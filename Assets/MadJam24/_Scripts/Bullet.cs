...
public class Bullet : MonoBehaviour
{
    ...
    public void SetPositionAndDirection(Transform rotationTransform, float speed, Vector3 overrideScale)
    {
        _isReady = false;
        ...
        var targetScale = _bulletData.BulletScale;
        
        if(overrideScale != Vector3.zero)
            targetScale = overrideScale;

        transform.DOScale(targetScale,  _bulletData.ScaleAnimDuration).SetEase( _bulletData.StartScaleEase);
